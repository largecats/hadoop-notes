...
**Data locality.** Make data access faster by co-locating data with compute nodes, so that don’t need to send data across network, which is expensive in data centers.
...
**Data locality optimization.** Hadoop tries to run the map task on a node that contains the input data split.
...
**Small file issue.** The number of files in a file system is limited by its name node memory. If there are a lot of small files in the file system, it needs a lot of name node memory to store their metadata.
